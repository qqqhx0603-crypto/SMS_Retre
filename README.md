...
- 可设为备用机默认短信 App，用 `SMS_DELIVER` 通道接收普通广播拿不到的验证码短信。
...
安装后打开 app，授予短信权限和通知权限。若要处理普通广播拿不到的验证码短信，需要在 app 内点击 `设为默认短信App`，并在系统弹窗里选择 `SMS-Retre`。
...
如果某些验证码短信能在系统短信 app 里看到，但 SMS-Retre 没有记录，可以让 SMS-Retre 接管备用机的默认短信 App 角色：

1. 在 SMS-Retre 里点击 `设为默认短信App`。
2. 在系统弹窗里选择 `SMS-Retre`。
3. 回到 SMS-Retre，确认页面显示 `默认短信 App：SMS-Retre`。
4. 再测试原来不能转发的验证码短信。

设为默认短信 App 后，SMS-Retre 会尽量把收到的短信写回系统短信库，避免备用机本机短信列表完全断掉。但它不是完整聊天软件，不适合当主力手机短信 App 使用。

### 5. Clash Meta for Android 设置
...
- 设为默认短信 App 后，SMS-Retre 会获得更高的短信处理权限，用于接收和写入备用机短信。
