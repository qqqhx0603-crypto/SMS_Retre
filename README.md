...
- 如果厂商系统没有广播某些验证码短信，可用“扫描最近短信”补漏转发。
...
安装后打开 app，授予短信接收、短信读取和通知权限。短信读取权限只用于扫描最近短信，补漏处理没有触发广播的验证码短信。
...
5. 如果某个平台验证码没有自动转发，但系统短信 app 里能看到，可以点击 `扫描最近短信`。
...
- `READ_SMS` 权限仅用于扫描最近短信补漏，不用于读取全量历史短信。
