...
- 可开启通知访问作为兜底：当个别验证码短信没有给第三方 app 普通短信广播、但系统短信 app 弹出通知时，SMS-Retre 会自动转发该通知内容。
...
如果某些验证码短信能在系统短信 app 里看到，但 SMS-Retre 没有记录，可以开启通知兜底：

1. 在 SMS-Retre 里点击 `打开通知访问设置`。
2. 在系统设置中找到并允许 `SMS-Retre` 的通知访问权。
3. 回到 SMS-Retre，确认页面里显示 `通知兜底：已开启`。

通知兜底只用于系统短信 app 的通知，并会等待普通短信广播先入库，避免正常短信重复转发。
