...
- 每 60 分钟定时扫描系统短信收件箱，发现未被 SMS-Retre 记录过的短信时，会汇总成一封邮件发送；没有漏转短信则不发送汇总邮件。
- 邮件中会标注配置的 SIM1/SIM2 接收号码或备注。
- 检测到备用机电量不高于 10% 时，会发送一次低电量提醒邮件。
...
**如果备用机是 vivo / OriginOS，请务必检查信息 App 的 `验证码安全保护`、`验证码保护` 或类似隐私保护开关。**

这个开关会禁止第三方应用读取和使用验证码短信，表现通常是：普通短信可以转发，但验证码短信不能转发。要转发验证码类隐私短信，需要在 vivo 信息 App 的隐私保护设置中关闭这个开关，然后再测试 SMS-Retre。

SMS-Retre 安装后第一次运行也会弹窗提醒这件事；主界面会显示简短使用教程。
...
如果 vivo 信息 App 里有 `验证码安全保护`、`验证码保护` 或类似开关，并且说明会禁止第三方应用读取或使用验证码信息，需要关闭它。否则普通短信能转发，但验证码短信可能不会给 SMS-Retre。

### 6. 定时漏转扫描

SMS-Retre 会默认每 60 分钟扫描一次系统短信收件箱，只处理系统短信里存在、但 SMS-Retre 本地记录里不存在的短信。扫描到漏转短信时，会发送一封汇总邮件，里面包含汇总信息和每一条短信的完整内容；没有扫描到漏转短信时不会发邮件。

为了避免第一次运行时把历史短信全发出去，首次扫描只看最近约 65 分钟的短信。汇总邮件发送成功后，这些短信会记入 SMS-Retre 本地记录，后续扫描不会重复发送。
...
- 定时漏转扫描需要 `READ_SMS` 权限，用来读取系统短信收件箱并补发漏转汇总。
