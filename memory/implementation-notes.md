...
- 2026-06-30: v0.4.0 makes SMS-Retre eligible as the default SMS app by adding SMS send-to/respond-via-message handlers plus an MMS delivery receiver. Incoming `SMS_DELIVER` messages are forwarded and then written to the system SMS inbox when SMS-Retre is the current default SMS app. A short duplicate window avoids double-forwarding if both `SMS_DELIVER` and `SMS_RECEIVED` arrive for the same message.
