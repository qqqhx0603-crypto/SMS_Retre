...
- 2026-06-30: v0.3.2 adds an automatic notification-listener fallback for SMS app notifications. It is meant for provider/app-specific verification SMS that show in the system SMS app but are not delivered to the app through `SMS_RECEIVED`. It filters to the default/known SMS app package, waits 12 seconds, and skips forwarding if the same body already entered the database through the normal SMS broadcast.
